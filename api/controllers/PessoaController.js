...
class PessoaController {
  ...
  static async pegaUmaMatricula(req, res) {
    ...
    try {
      ...
      const umaMatricula = await pessoa.getAulasMatriculadas();
  ...
  static async pegarMatruiculasPorTurmas(req, res) {
    const { id } = req.params
    try {
      const todasMatriculas = database.Matriculas.findAndCountAll({
        where: {
          turma_id: Number(id),
          status: 'confirmado'
        }
      })
      res.status(200).json(todasMatriculas)
    } catch (error) {
      return res.status(500).json(error.message)
    }
  }
